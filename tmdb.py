...
class Movie:
    def __init__(
        self, id: str, name: str, poster: str = "http://127.0.0.1:7000/static/img/9.jpg"
    ):
...
def get_imdb_id(tmdb_id: int) -> str:
    query = (
        f"https://api.themoviedb.org/3/movie/{tmdb_id}/external_ids?api_key={TMDB_KEY}"
    )
...
def get_movies_filtered(
    filters: dict[str, str | int], amount: int = 20, start_page=1
) -> list[Movie]:
    movies = []
    page_number = start_page
    ...
    while len(movies) < amount:
        ...
        response = requests.get(
            url,
            params=(
                {"api_key": TMDB_KEY, "sort_by": "popularity.desc", "page": page_number}
                | filters
            ),
        )
        response_dict = response.json()
...
def get_random_movie() -> Movie:
    ...
    response = requests.get(
        url, params={"api_key": TMDB_KEY, "sort_by": "popularity.desc", "page": page}
    ).json()
